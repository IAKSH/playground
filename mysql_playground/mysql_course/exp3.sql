...
INSERT INTO tb VALUES (1,20),(2,30);

DELIMITER $$
CREATE TRIGGER trigger_upd_check BEFORE UPDATE
ON tb FOR EACH ROW 
BEGIN
	IF new.a<0 THEN SET new.a=0;
	ELSEIF new.a>100 THEN SET new.a=101;
	END IF;
END$$

UPDATE tb SET a=150 WHERE id=1;

-- 3.5
SELECT * FROM tb;

-- 3.6
-- 在tb表被更新前执行，用于检查新的a值，如果小于0则设置为0，如果大于100则设置为101

-- 3.9
DELIMITER $$
CREATE TRIGGER trigger_borrow_insert BEFORE INSERT
ON borrow FOR EACH ROW 
BEGIN
	UPDATE reader SET reader.borrowed_num = reader.borrowed_num + 1
	WHERE reader.readerno = new.readerno;
	UPDATE book SET book.state = "借出" WHERE book.bookno = new.bookno;
END$$

INSERT INTO borrow VALUES ("2225","TP273-342",NOW(),NULL);