...
auto logger = spdlog::stdout_color_mt("yolo_detector");
...
void YOLODetector::initConfig(const std::string& onnxpath, int iw, int ih, float threshold, bool isCuda) {
    ...
    if(isCuda){
        logger->info("Intialize Model By GPU");
        this->net.setPreferableBackend(cv::dnn::DNN_BACKEND_CUDA);
        this->net.setPreferableTarget(cv::dnn::DNN_TARGET_CUDA);
    } else {
        logger->info("Intialize Model By CPU");
...
//    frame = resize_max_edge(frame, this->input_h);
    ...
    cv::Mat channel0(blob.size[2], blob.size[3], CV_32F, blob.ptr<float>(0,0));
    cv::imwrite("channel0.jpg", channel0 * 255);
