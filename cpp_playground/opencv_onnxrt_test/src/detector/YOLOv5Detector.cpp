...
static const std::string LABELS_LIST_PATH = R"(E:\repos\playground\cpp_playground\opencv_onnxrt_test\test_labels_list.txt)";
...
void YOLOv5Detector::init(std::string onnxpath)
{
    ...
    std::string file = LABELS_LIST_PATH;
...
void YOLOv5Detector::detect(cv::Mat & frame, std::vector<detect_result> &results)
{
    ...
    //for (int i = 0; i < this->net.getLayerNames().size(); i++) {
    //    std::cout << this->net.getLayerNames()[i] << std::endl;
    //}


    cv::Mat preds = this->net.forward("output0");//outputname
...
void YOLOv5Detector::draw_frame(cv::Mat & frame, std::vector<detect_result> &results)
{
    ...
    for(auto dr : results)
    {
        ...
        label = classes_[dr.class_id ] + ":" + label;
