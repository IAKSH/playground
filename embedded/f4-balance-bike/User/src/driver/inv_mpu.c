...
#include <string.h>
#include <math.h>
#include "driver/inv_mpu/inv_mpu.h"
...
static HAL_StatusTypeDef i2c_read_impl(uint16_t dev_addr,uint16_t reg_addr,uint16_t data_size,uint8_t *p_data) {
    osMutexAcquire(i2c_bus_mutex,osWaitForever);
...
#define i2c_write(dev_addr, reg_addr, data_size,p_data) \
    i2c_write_impl(dev_addr,reg_addr,data_size,p_data)

#define i2c_read(dev_addr, reg_addr, data_size, p_data) \
    i2c_read_impl(dev_addr,reg_addr,data_size,p_data)

#define delay_ms osDelay
#define get_ms(p) do{ *p = osKernelGetTickCount();}while(0)
